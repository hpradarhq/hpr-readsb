...
ARG FE_VERSION=4.7.5-edge.rc12
ARG FE_BUILD=260916.rc12

FROM debian:bookworm-slim AS builder

RUN apt-get update && apt-get install -y --no-install-recommends \
      build-essential ca-certificates pkg-config wget librtlsdr-dev libusb-1.0-0-dev zlib1g-dev libzstd-dev \
...
RUN make clean \
    ...
    && wget --timeout=20 --tries=4 --retry-connrefused -O /out/aircraft.csv.gz https://raw.githubusercontent.com/wiedehopf/tar1090-db/csv/aircraft.csv.gz

FROM debian:bookworm-slim
ARG FE_VERSION
ARG FE_BUILD
RUN apt-get update && apt-get install -y --no-install-recommends \
      ca-certificates nginx tzdata jq busybox librtlsdr0 libusb-1.0-0 zlib1g libzstd1 \
    && rm -rf /var/lib/apt/lists/* \
...
RUN sed -i \
      ...
      -e "s|V4.7 LIVE / AIRWIRE|${FE_VERSION}|" \
...
ENV READSB_JSON_INTERVAL=1 READSB_GAIN=auto HPR_UPSTREAM_PORT=30004 HPR_AIRWIRE_WS_PORT=30154 HPR_EDGE_DATA_DIR=/data/hpr-edge
EXPOSE 80 30002 30003 30004 30005
HEALTHCHECK --interval=30s --timeout=3s --start-period=30s --retries=3 CMD test -s /run/readsb/airwire.json || exit 1
