...
function buildUi(){
  ...
  const bar=document.createElement('div');bar.id='hprOpsBar';bar.innerHTML=`
    ...
    <div id="hprFilterDrawer" hidden>
      ...
      <div class="hpr-actions"><button id="hprClear">Clear</button><button id="hprAllTracks">All tracks 60m</button><button id="hprCoverage">Coverage</button></div>
    </div>`;
  tabs.after(bar);
  ...
  #hprFilterDrawer{padding:5px 7px 7px;border-top:1px solid var(--border)}.hpr-grid{display:grid;grid-template-columns:1fr 1fr;gap:5px}.hpr-grid label{font-size:8px;color:var(--muted);text-transform:uppercase;letter-spacing:.35px}.hpr-grid input,.hpr-grid select{display:block;width:100%;height:28px;margin-top:2px;border:1px solid var(--border);border-radius:5px;background:var(--surface);color:var(--text);padding:0 6px;font:10px system-ui}.hpr-actions{display:grid;grid-template-columns:1fr 1fr 1fr;gap:5px;margin-top:6px}
...
function syncControls(){
  ...
  $('#hprAllTracks')?.classList.toggle('on',state.allTracks);$('#hprCoverage')?.classList.toggle('on',state.rings);
}
...
function applyRows(){
  ...
  const byId=new Map(rows.map(r=>[r.dataset.id,r]));for(const id of state.ids){const r=byId.get(id);if(r)list.appendChild(r)}
...
function readUrl(){
  const q=new URLSearchParams(location.search);state.type=q.get('type')||'';state.altMin=q.get('altMin')||'';state.altMax=q.get('altMax')||'';state.source=q.get('source')||'all';state.sort=q.get('sort')||'seen';state.units=q.get('units')||'nautical';state.preset=q.get('preset')||'';state.selectedFromUrl=(q.get('icao')||'').toLowerCase()||null;state.allTracks=q.get('allTracks')==='1';state.rings=q.get('coverage')!=='0';
...
function updateUrl(){
  const q=new URLSearchParams();if(state.type)q.set('type',state.type);if(state.altMin!=='')q.set('altMin',state.altMin);if(state.altMax!=='')q.set('altMax',state.altMax);if(state.source!=='all')q.set('source',state.source);if(state.sort!=='seen')q.set('sort',state.sort);if(state.units!=='nautical')q.set('units',state.units);if(state.preset)q.set('preset',state.preset);if(state.allTracks)q.set('allTracks','1');if(!state.rings)q.set('coverage','0');const sel=$('#airList .row.selected')?.dataset.id;if(sel)q.set('icao',sel);const m=window.HPREdgeMap;if(m){const c=m.getCenter();q.set('lat',c.lat.toFixed(4));q.set('lon',c.lng.toFixed(4));q.set('zoom',m.getZoom().toFixed(2))}const s=q.toString(),u=location.pathname+(s?'?'+s:'');if(u!==state.lastUrl){state.lastUrl=u;history.replaceState(null,'',u)}}
...
function observe(){new MutationObserver(()=>{applyRows();applyUnits();decorateHealth();updateUrl()}).observe(document.body,{childList:true,subtree:true});setInterval(snapshot,1000);setInterval(health,15000);setInterval(()=>{if(state.allTracks)loadAllTracks();if(state.rings)renderCoverage()},300000)}
