...
function applyPersistentDisplay(){
  ...
  const u=new URLSearchParams(location.search);if(!u.has('units')){const el=$('#hprUnits');if(el&&el.value!==cfg.display.units){el.value=cfg.display.units;el.dispatchEvent(new Event('change',{bubbles:true}))}}
  applyRings();
}
function applyRings(){
  ...
  try{
    ...
    m.setLayoutProperty('hpr-admin-rings','visibility',d.ring_enabled?'visible':'none');
    if(m.getLayer('hpr-actual-range'))m.setLayoutProperty('hpr-actual-range','visibility',d.actual_range?'visible':'none');
  }catch(_){}
}
function hookMap(){const m=window.HPREdgeMap;if(!m||hookedMap)return;hookedMap=true;m.on?.('style.load',()=>setTimeout(applyRings,100));applyRings()}
function feederRows(){
...
function stationPayload(p){
  const lat=$('#hprSetLat').value.trim(),lon=$('#hprSetLon').value.trim(),height=$('#hprSetHeight').value.trim();
  return {pin:p,name:$('#hprSetName').value.trim(),lat:lat===''?null:num(lat),lon:lon===''?null:num(lon),height_m:height===''?null:num(height),uuid:$('#hprSetUuid').value.trim()}
}
...
function feederPayload(p,enabled=true){const id=$('#hprFeedId').value||`f_${crypto.randomUUID().slice(0,8)}`;return {pin:p,id,name:$('#hprFeedName').value.trim(),host:$('#hprFeedHost').value.trim(),port:Number($('#hprFeedPort').value),protocol:$('#hprFeedProto').value,enabled,uuid:$('#hprFeedUuid').value.trim()}}
async function refreshUi(){await load();const old=$('#hprPersistentSettings');old?.remove();renderSettings();hookMap()}
function bind(root){
  $('#hprGenUuid').onclick=()=>{$('#hprSetUuid').value=crypto.randomUUID()};
...
function style(){if($('#hprAdminStyle'))return;const s=document.createElement('style');s.id='hprAdminStyle';s.textContent=`
  .hpr-admin-grid{display:grid;grid-template-columns:1fr 1fr;gap:8px}.hpr-admin-field>span{display:block;color:var(--muted);font-size:9px;text-transform:uppercase;letter-spacing:.4px;margin-bottom:3px}.hpr-admin-field input,.hpr-admin-field select{width:100%;height:31px;border:1px solid var(--border);border-radius:5px;background:var(--s2);color:var(--text);padding:0 7px;font:11px system-ui}.hpr-admin-field small{display:block;color:var(--muted);font-size:8px;margin-top:2px}.hpr-inline{display:flex;gap:5px}.hpr-inline input{min-width:0;flex:1}.hpr-inline button,.hpr-feeder button{border:1px solid var(--border);border-radius:5px;background:var(--s2);color:var(--text2);font-size:9px;padding:0 7px}.hpr-switchline{height:31px;display:flex;align-items:center}.hpr-feeders{margin-bottom:8px}.hpr-feeder{min-height:42px;display:grid;grid-template-columns:1fr auto auto auto;gap:6px;align-items:center;border-bottom:1px solid var(--border)}.hpr-feeder small{display:block;color:var(--muted);font-size:8px}.hpr-feeder>button{height:27px}.hpr-wide{width:100%;margin-top:8px}.hpr-admin-msg{min-height:18px;margin-top:7px;font-size:9px}.hpr-admin-msg.good{color:var(--live)}.hpr-admin-msg.bad{color:var(--danger)}
  @media(max-width:680px){.hpr-admin-grid{grid-template-columns:1fr}}
...
document.addEventListener('DOMContentLoaded',()=>{style();observe();load().then(()=>{hookMap();renderSettings();setInterval(()=>{hookMap();applyRings()},5000)}).catch(()=>{})},{once:true});
window.HPREdgeAdmin=Object.freeze({config:()=>cfg,reload:load,applyRings});
})();
